...
def vehicles():
    ...
    if request.method == "POST":
        ...
        """, (
            ...
            request.form.get("four_day_price") or 0,
            request.form.get("weekend_price") or 0,
            request.form.get("monthly_price") or 0,
            request.form.get("deposit_short") or 0,
            request.form.get("deposit_month") or 0,
            request.form.get("km_day") or 150,
            request.form.get("km_offer") or 350,
            request.form.get("km_month") or 1000,
            request.form.get("status", "Frei"),
            request.form.get("image_link", ""),
            request.form.get("color_tag", ""),
...
def customers():
    ...
    if request.method == "POST":
        ...
        """, (
            ...
            request.form.get("license_no", ""),
            request.form.get("license_checked", "Nein"),
            request.form.get("license_link", ""),
            request.form.get("id_link", ""),
            request.form.get("note", "")
        ))
        db.commit()
...
def customer_edit(customer_id):
    ...
    if request.method == "POST":
        cur.execute("""
            UPDATE customers
            SET first_name=%s, last_name=%s, phone=%s, email=%s, address=%s, birth_date=%s,
                license_no=%s, license_checked=%s, license_link=%s, id_link=%s, note=%s
            WHERE id=%s
        """, (
            request.form.get("first_name", ""),
            request.form.get("last_name", ""),
...
def customer_delete(customer_id):
    ...
    cur.execute("SELECT COUNT(*) AS c FROM bookings WHERE customer_id=%s", (customer_id,))
    booking_count = cur.fetchone()["c"]

    cur.execute("SELECT COUNT(*) AS c FROM damages WHERE customer_id=%s", (customer_id,))
    damage_count = cur.fetchone()["c"]
...
def bookings():
    ...
    if request.method == "POST":
        ...
        """, (
            ...
            request.form.get("special_price") or None,
            request.form.get("special_reason", ""),
            request.form.get("extra_km") or 0,
            request.form.get("deposit_status", "nicht bezahlt"),
    ...
    cur.execute("SELECT id, name FROM vehicles ORDER BY name")
    ...
    for row in rows:
        ...
        items.append({
            "row": row,
            "calc": calc,
            "contract_no": contract_no(row["id"]),
            "invoice_no": invoice_no(row["id"])
        })
...
def booking_edit(booking_id):
    ...
    if request.method == "POST":
        cur.execute("""
            UPDATE bookings
            SET vehicle_id=%s, customer_id=%s, start_date=%s, pickup_time=%s, end_date=%s, return_time=%s,
                special_price=%s, special_reason=%s, extra_km=%s, deposit_status=%s, pickup_km=%s, return_km=%s,
                pickup_fuel=%s, return_fuel=%s, note=%s
            WHERE id=%s
        """, (
            request.form.get("vehicle_id", ""),
            request.form.get("customer_id", ""),
...
def calendar():
    ...
    cur.execute("SELECT * FROM bookings WHERE vehicle_id=%s", (vehicle_id,))
